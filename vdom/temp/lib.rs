    ...
    TChildren: IElementProps,
{
    ...
    pub fn attributes(self) -> Vec<TAttributes> {
        self.attributes
    }

    pub fn children(self) -> Vec<TChildren> {
        self.children
    }
}
...
impl From<Language> for HtmlAttributes {
    fn from(source: Language) -> HtmlAttributes {
        HtmlAttributes::Language(source)
    }
}

impl<'a> From<&'a Language> for HtmlAttributes {
    fn from(source: &Language) -> HtmlAttributes {
        HtmlAttributes::Language(source.to_owned())
    }
}
...
impl<'a> IAttribute for &'a HtmlAttributes {}

impl<'a> From<&'a HtmlAttributes> for Box<IAttribute> {
    fn from(attributes: &'a HtmlAttributes) -> Box<IAttribute> {
...
pub type Html = Element<HtmlAttributes, HtmlChildren>;
...
pub trait IHtmlChildren: std::fmt::Debug {}

impl IElement for HtmlChildren {}

impl<'a> IElement for &'a HtmlChildren {}

impl IHtmlChildren for HtmlChildren {}

impl<'a> From<&'a HtmlChildren> for Box<IElement> {
    fn from(children: &'a HtmlChildren) -> Box<IElement> {
        Box::new(children.to_owned())
    }
}

impl<'a> From<HtmlChildren> for Box<IElement> {
    fn from(children: HtmlChildren) -> Box<IElement> {
        Box::new(children)
    }
}

impl From<Element<BodyAttributes, BodyElements>> for HtmlChildren {
    fn from(element: Element<BodyAttributes, BodyElements>) -> HtmlChildren {
        HtmlChildren::Body(element)
    }
}
pub fn html<TAttributes, TChildren>(attributes: &[TAttributes], children: &[TChildren]) -> Html
where
    TAttributes: IHtmlAttribute + Clone,
    TChildren: IHtmlChildren + Clone,
    HtmlAttributes: From<TAttributes>,
    HtmlChildren: From<TChildren>,
{
    let attributes: Vec<HtmlAttributes> = attributes.into_iter().map(|item| (*item).to_owned().into()).collect();
    let children: Vec<HtmlChildren> = children.into_iter().map(|item| (*item).to_owned().into()).collect();
...
impl IHtmlChildren for BodyElements {}
...
impl IHtmlChildren for Element<BodyAttributes, BodyElements> {}
...
impl<'a> From<&'a Attribute> for HtmlAttributes {
    fn from(source: &Attribute) -> HtmlAttributes {
        HtmlAttributes::Attribute(source.to_owned())
    }
}
...
mod tests {
    ...
    fn should() {
        ...
        let element: Html = html(
            &[  Language { name: "en".to_owned() }.into()
...
// impl From<Element<HtmlAttributes, HtmlChildren>> for Element<Box<IAttribute>, Box<IElement>> {
//     fn from(element: Element<HtmlAttributes, HtmlChildren>) -> Element<Box<IAttribute>, Box<IElement>> {
...
// // std::convert::From<Element<std::boxed::Box<(dyn IAttribute + 'static)>, std::boxed::Box<(dyn IElement + 'static)>>>` is not implemented for `Element<HtmlAttributes, HtmlChildren>`
// impl From<Element<Box<IAttribute>, Box<IElement>>> for Element<HtmlAttributes, HtmlChildren> {
//     fn from(element: Element<Box<IAttribute>, Box<IElement>>) -> Element<HtmlAttributes, HtmlChildren> {
//         // let attributes: Vec<HtmlAttributes> = element.attributes.into_iter().map(|item| item.to_owned().into()).collect();
//         let attributes: Vec<HtmlAttributes> = element.attributes.into_iter().map(|item| item.into()).collect();
//         let children: Vec<HtmlChildren> = element.children.into_iter().map(|item| item.to_owned().into()).collect();
//         Element::new(element.kind, attributes, children)
//     }
// }
